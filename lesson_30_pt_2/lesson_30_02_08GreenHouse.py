...
async def read_temperature_sensor(task_num):
    ...
    print(f'Считывание данных с датчика температуры...')
    read_time = random.uniform(1, 3)
    await asyncio.sleep(read_time)
    temp = random.uniform(18, 30)
    print(f'Температура в теплице {task_num}: {temp:.2f}°C >> t сбора {read_time:.2f} секунд ')
    return temp


async def read_humidity_sensor(task_num):
    """Имитация считывания данных с датчика влажности."""
    print(f'Считывание данных с датчика влажности...')
    read_time = random.uniform(2, 4)
    await asyncio.sleep(read_time)
    humidity = random.uniform(40, 70)
    print(f'Влажность в теплице {task_num}: {humidity:.2f}% >> t сбора {read_time:.2f} секунд ')
    return humidity


async def monitoring_climate(times):
    """Мониторинг данных с датчиков температуры и влажности."""
    print(datetime.now())
    temperature_tasks = [read_temperature_sensor(i) for i in range(1, times + 1)]
    humidity_tasks = [read_humidity_sensor(i) for i in range(1, times + 1)]
    ...
    middle_data = len(results) // 2

    temperature = results[:middle_data]
    humidity = results[middle_data:]

    print(f'Средняя температура: {sum(temperature) / len(temperature):.2f}°C')
    print(f'Средняя влажность: {sum(humidity) / len(humidity):.2f}%')

    print(datetime.now())


if __name__ == '__main__':
    asyncio.run(monitoring_climate(5))
